	...
	<section class="card">
		...
		<div class="setting-row">
			<div class="setting-label">
				<h3>Measurements</h3>
				<p>Wave height and temperature units</p>
			</div>
			<div class="radio-group">
				<label>
					<input
						...
						onchange={() => saveUnitSettings({ unit_system: 'metric' })}
						{unitSaving}
					/>
					<span>Metric (m, °C)</span>
				</label>
				<label>
					<input
						type="radio"
						...
						onchange={() => saveUnitSettings({ unit_system: 'imperial' })}
						{unitSaving}
					/>
					<span>Imperial (feet, °F)</span>
				</label>
			</div>
		</div>
			...
			<div class="radio-group">
				<label>
					<input
						...
						onchange={() => saveUnitSettings({ time_format: '24h' })}
						{unitSaving}
					/>
				...
				<label>
					<input
						...
						onchange={() => saveUnitSettings({ time_format: '12h' })}
						{unitSaving}
					/>
		...
		<div class="setting-row" style="margin-top:4px">
			<div class="setting-label">
				<h3>Boat icon</h3>
				<p>Shown on the public tracking map</p>
			</div>
		</div>
		...
		{#if unitMessage}
			<div class="alert" class:alert-success={unitMessage.includes('saved')} class:alert-error={unitMessage.includes('Error')}>
				{unitMessage}
			</div>
		{/if}
	</section>
...
<style>
	...
	.setting-row {
		...
		gap: 20px; padding: 12px 0; border-bottom: 1px solid var(--border);
	}
	.setting-row:last-child { border-bottom: none; padding-bottom: 0; }

	.setting-label { flex: 1; }
	.setting-label h3 { font-size: 13px; font-weight: 600; color: var(--text); margin: 0 0 3px; }
	.setting-label p { font-size: 11px; color: var(--muted); margin: 0; }

	.radio-group { display: flex; flex-direction: column; gap: 6px; min-width: 180px; }
	.radio-group label {
		display: flex; align-items: center; gap: 6px; cursor: pointer;
		font-size: 13px; color: var(--text); padding: 4px;
	}
	.radio-group input[type='radio'] { cursor: pointer; width: 14px; height: 14px; margin: 0; }
	...
	.boat-icon-btn:hover { background: rgba(0,200,255,0.06); color: var(--accent); }
	.boat-icon-btn.selected {
